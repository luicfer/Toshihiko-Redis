...
var Redis = function(servers, options) {
    ...
    this.redis = new _Redis.createClient(servers[1],servers[0], this.options);
 ...
 * @param {String} the keyname in redis
 */
...
Redis.prototype.getData = function(dbName, tableName, keys, callback) {
    ...
    multi.exec(function (err,res) {
        res = res.filter(function(it){return it !== null});
        res = res.map(function(it){return JSON.parse(it)});
        callback(err,res)
    });
};
